...
- **مستندسازی:** جزئیات سیاست Soft Delete در فایل `docs/technical/SOFT_DELETE.md` ثبت شد.
...
### اقدام انجام شده:
- اسکریپت `infrastructure/scripts/verify-db.sh` برای بررسی خودکار صحت دیتابیس و Migrationها در CI/CD اضافه شد.
...
- **اسکریپت تأیید خودکار تنظیمات:** اسکریپت `infrastructure/scripts/verify-production-config.ts` اضافه شد که قبل از Deployment تمام متغیرهای env و اتصالات را چک می‌کند (قابل اجرا با `npm run verify:prod`).
