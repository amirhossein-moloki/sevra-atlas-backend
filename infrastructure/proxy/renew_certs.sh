...
docker compose -f infrastructure/docker/docker-compose.prod.yml run --rm certbot renew >> "$LOG_FILE" 2>&1

# Reload Nginx to pick up any new certificates
docker compose -f infrastructure/docker/docker-compose.prod.yml exec -T nginx nginx -s reload >> "$LOG_FILE" 2>&1
