  ...
  responses: {
    200: {
      ...
      content: { 'application/json': { schema: withApiSuccess(z.object({
        ...
        name: z.string(),
        status: z.string(),
        progress: z.record(z.any()).nullable().openapi({ type: 'object' }),
        failedReason: z.string().optional(),
        timestamp: z.number(),
        finishedOn: z.number().optional(),
        data: z.record(z.any()).nullable().openapi({ type: 'object' }),
        returnValue: z.record(z.any()).nullable().openapi({ type: 'object' }),
      })) } },
    },
  },
});
