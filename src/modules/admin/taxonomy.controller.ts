...
export class AdminTaxonomyController {
  async reorderBlogCategories(req: Request, res: Response) {
    await blogTaxonomyService.reorderCategories(req.body.items);
    res.json({ ok: true });
  }

  async reorderServiceCategories(req: Request, res: Response) {
    await servicesService.reorderCategories(req.body.items);
    res.json({ ok: true });
  }

  async reorderSpecialties(req: Request, res: Response) {
    await artistsService.reorderSpecialties(req.body.items);
    res.json({ ok: true });
  }
}
