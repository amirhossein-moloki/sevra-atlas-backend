...
export class FollowsController {
  async follow(req: AuthRequest, res: Response) {
    ...
    const result = await followsService.follow(req.user!.id, targetType as FollowTargetType, safeBigInt(targetId, 'targetId'));
    res.json(result);
  }

  async unfollow(req: AuthRequest, res: Response) {
    const { targetType, targetId } = req.body;
    const result = await followsService.unfollow(req.user!.id, targetType as FollowTargetType, safeBigInt(targetId, 'targetId'));
    res.json(result);
  }
