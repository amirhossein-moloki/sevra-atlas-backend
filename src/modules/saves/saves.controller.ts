...
export class SavesController {
  async save(req: AuthRequest, res: Response) {
    ...
    const result = await savesService.save(req.user!.id, targetType as SaveTargetType, safeBigInt(targetId, 'targetId'));
    res.json(result);
  }

  async unsave(req: AuthRequest, res: Response) {
    const { targetType, targetId } = req.body;
    const result = await savesService.unsave(req.user!.id, targetType as SaveTargetType, safeBigInt(targetId, 'targetId'));
    res.json(result);
  }
